...
result_file = out / 'v123_results.json'
...
names.extend([
    ...
    'validate_v118_evolution_hands.py',
    'validate_v119_overview_scroll.py',
    'validate_v120_connection.py',
    'validate_v123_evolution_recovery.py',
    'validate_v123_evolution_placement.py',
])
for name in names:
...
run('evolution_initial_state_test.cjs',
    lambda: command(['node', str(root / 'tests/evolution_initial_state_test.cjs')]))
run('debug_profile_cleanup_test.ps1',
    lambda: command(['powershell.exe','-NoProfile','-ExecutionPolicy','Bypass','-File',str(root / 'tests/debug_profile_cleanup_test.ps1')]))
run('evolution_continuity_test.cjs',lambda: command(['node',str(root/'tests/evolution_continuity_test.cjs')]))
