...
previous = json.loads((out/'v114_results.json').read_text(encoding='utf-8')) if '--failed' in sys.argv and (out/'v114_results.json').exists() else []
...
def run(name, action):
    ...
    (out/'v114_results.json').write_text(json.dumps(results,ensure_ascii=False,indent=2),encoding='utf-8')
def command(args):
...
names.extend(['validate_v108_chip_selector_transport.py', 'validate_v110_configured_export_model.py', 'validate_v111_tie_evolution_telegram.py', 'validate_v112_betting_window.py', 'validate_v113_viewport.py', 'validate_v113_signal_accounting.py', 'validate_v114_default_mode.py'])
