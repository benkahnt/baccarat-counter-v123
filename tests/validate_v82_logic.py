...
assert 'rowCells({220, 190, 160, 135, 150, 135})' in layout
assert 'rowCells({105, 70, 75, 185, 165})' in layout
assert 'rowCells({210, 285, 320, 475})' in layout
assert 'moveLabeledCell(a->pairChipLabel, a->pairChipEdit, row[4], 92, 30);' in layout
assert 'moveLabeledCell(a->stopLossLabel, a->stopLossEdit, row[5], 72, 30);' in layout
assert 'moveCell(a->evolutionRollingBtn, row[1]);' in layout
assert 'moveLabeledCell(a->minEdgeLabel, a->minEdgeEdit, row[2], 92, 30);' in layout
assert 'moveLabeledCell(a->unitTargetLabel, a->unitTargetEdit, row[3], 72, 30);' in layout
...
assert 'deadline:Date.now()+Math.max(9000,3500+MAIN_CHIP_CLICKS*220+PAIR_CHIP_CLICKS*220)' in src
...
assert 'const std::string& inputSessionId = pending.sessionId;' in src
assert 'const std::string inputSessionId = PragmaticCurrentBetifyPageSession();' not in take('    void HandlePragmaticTrustedPairClickResponse(', '    void Bootstrap()')
assert 'foregrounded-embedded-pragmatic' in src
assert 'PRAGMATIC_PAPER_BET_SKIPPED_NO_CONFIRMED_CHIP' in src
print('v81 foregrounded Betify window + embedded Pragmatic trusted click + confirmation-only accounting OK')
...
assert 'Betify-Page-Session für Top-Level-Koordinaten fehlt' not in trusted
assert 'foregrounded-embedded-pragmatic' in trusted
assert 'foregrounded-betify-page' not in trusted
...
print('v81 restored v63/v65 Pragmatic trusted input + compact status + rollback-safe SETZEN counter OK')
...
# v21.2.78: Fresh-start Pragmatic OOPIF recovery must be additive only.
assert 'std::vector<std::string> PragmaticRecoveryRuntimeSessions(int recoveryStage)' in src
assert 'if (recoveryStage < 5 || recoveryStage > 6)' in src
assert 'if (directDesktopRuntimeKnown)' in src
assert 'Unknown/blank iframes are kept' in src
assert 'sessions = PragmaticRecoveryRuntimeSessions(recoveryStage);' in src
assert 'blank-url session may have been skipped' in src
assert 'Target.setAutoAttach' in src

# The exact long-proven Stage-5 MULTIPLAY locator/click block must remain byte-for-byte
# equivalent to v21.2.77. Hashing the source block keeps this validator portable.
import hashlib
_stage5_a = '    // Stage 5: in the Pragmatic primary Baccarat document click only the exact\n'
_stage5_b = '    // Stage 6: success is only declared when the Multiplay document itself is\n'
_i = src.index(_stage5_a)
_j = src.index(_stage5_b, _i)
_stage5 = src[_i:_j]
assert hashlib.sha256(_stage5.encode('utf-8')).hexdigest() == 'b7c7854b044023e27ef87806bd1ab5eba737c6159197fa76590a491c5eac106a'
print('v21.2.78 fresh-start MULTIPLAY recovery regression retained: OK')


# v21.2.79: dynamic Pragmatic Multiplay membership + compact yellow status.
update_stats=take('static void UpdateStats(AppState* a)',
                  'static void Layout(HWND hwnd, AppState* a)')
assert 'Startbalance:' not in update_stats
assert 'Aktuelle Balance:' not in update_stats
assert 'Endbalance:' not in update_stats
assert 'Balance vor Verbinden:' not in update_stats
assert 'activeTableCount' in update_stats
assert 'visibleFilterIds.find(kv.first)' in update_stats
assert 'SETZEN-Signale: ' in update_stats

probe_console=take('        if (auto value = extractConsoleValue("__BAC_PRAGMATIC_PROBE__")) {',
                   '        if (auto value = extractConsoleValue("__BAC_PRAGMATIC_PROBE_ERROR__")) {')
for required in (
    'PRAGMATIC_MULTIPLAY_TABLE_APPEARED',
    'pragmaticMissingNameProbeCount_',
    'pragmaticMissingNameSinceTick_',
    'misses >= 3 && age >= 16000ULL',
    'PragmaticDeactivateTableFromMultiplay(',
    'PragmaticReactivateTableFromMultiplay(',
    'PragmaticHasConfiguredTableName(name)',
    'probes >= 3 && age >= 16000ULL',
    'PragmaticRequestMultiplayFeedRefresh(unresolvedNames)',
    'PostPragmaticVisibleFilterSnapshot();'):
    assert required in probe_console

assert 'pragmaticInactiveTableIds_' in src
assert 'PRAGMATIC_MULTIPLAY_TABLE_DEACTIVATED' in src
assert 'PRAGMATIC_MULTIPLAY_TABLE_REACTIVATED' in src
assert 'PRAGMATIC_SETZEN_SIGNAL_INVALIDATED_TABLE_UNAVAILABLE' in src
assert 'PRAGMATIC_DYNAMIC_TABLE_FEED_REFRESH' in src
assert 'now - pragmaticLastDynamicFeedRefreshTick_ < 60000ULL' in src
assert 'Page.reload", "{\\"ignoreCache\\":false}", multiSession' in src
assert 'visible-table-without-tableconfig' in src
assert 'multibaccarat-session-not-found' in src

# Inactive tables must be blocked before any live event can mutate shoe/signal state.
payload=take('    void HandlePragmaticPayload(const std::string& payload) {',
             '    void Log(const std::wstring& msg) {')
assert 'pragmaticInactiveTableIds_.find(*payloadTableId)' in payload
assert 'if (!visibleInMultiplay)' in payload
assert 'PostPragmaticVisibleFilterSnapshot();' in payload

# The long-proven Stage-5 MULTIPLAY locator hash is still exactly unchanged.
_i = src.index(_stage5_a)
_j = src.index(_stage5_b, _i)
assert hashlib.sha256(src[_i:_j].encode('utf-8')).hexdigest() == 'b7c7854b044023e27ef87806bd1ab5eba737c6159197fa76590a491c5eac106a'
print('v21.2.79 dynamic Pragmatic table sync + status cleanup regression retained: OK')


# v21.2.81: stale/blank Pragmatic OOPIF URLs must not strand DOM actions.
assert 'void PragmaticRememberRuntimeSession(' in src
assert 'std::string pragmaticRuntimeSession_;' in src
...
assert 'providerSessions_.find(pragmaticRuntimeSession_) != providerSessions_.end()' in src
assert 'return {pragmaticRuntimeSession_};' in src
assert 'proof == "multiplay-control"' in src
assert 'proof == "multiplay-ready"' in src
assert 'proof == "focus-applied"' in src
assert 'proof == "keepalive"' in src
assert 'fullSessionId, probeRootHref, "probe-root-href"' in src
assert 'if (pragmaticRuntimeSession_ == *sid)' in src and 'pragmaticRuntimeSession_.clear();' in src
assert 'PragmaticRecoveryRuntimeSessions(5);' in src

# Stage-5 MULTIPLAY button search remains byte-identical.
_i = src.index(_stage5_a)
_j = src.index(_stage5_b, _i)
assert hashlib.sha256(src[_i:_j].encode('utf-8')).hexdigest() == 'b7c7854b044023e27ef87806bd1ab5eba737c6159197fa76590a491c5eac106a'
print('v21.2.82 persistent Pragmatic runtime + v63/v65 trusted-click restore regression: OK')


# v21.2.82: Good Roads must never masquerade as the complete Multiplay population.
...
# Normal UI no longer creates the two obsolete buttons.
assert 'L"BUTTON", L"Fokus"' not in src
assert 'L"BUTTON", L"Simulator Auto: AUS"' not in src
assert 'a->manualFocusBtn = nullptr;' in src
assert 'a->simulatorAutoBtn = nullptr;' in src
# The proven v63/v65 trusted input transport stays untouched.
trusted=take('    void HandlePragmaticTrustedPairClickResponse(', '    void Bootstrap()')
assert 'const std::string& inputSessionId = pending.sessionId;' in trusted
assert 'foregrounded-embedded-pragmatic' in trusted
assert 'foregrounded-betify-page' not in trusted
print('v21.2.82 All-Tables guard + UI cleanup regression: OK')
