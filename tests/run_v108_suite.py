...
previous = json.loads((out/'v108_results.json').read_text(encoding='utf-8')) if '--failed' in sys.argv and (out/'v108_results.json').exists() else []
...
def run(name, action):
    ...
    (out/'v108_results.json').write_text(json.dumps(results,ensure_ascii=False,indent=2),encoding='utf-8')
def command(args):
